...
var displayMap = function displayMap(locations) {
  mapboxgl.accessToken = 'pk.eyJ1IjoibWttb25pciIsImEiOiJjbDBnemgyMnkwMm1wM2NubWs3am5yZ3YzIn0.ztxtZO5zprJP4x4AjqXQOA';
  var map = new mapboxgl.Map({
    container: 'map',
    style: 'mapbox://styles/mapbox/light-v10',
    scrollZoom: false // center: [-118.113491, 34.11745],
    // zoom: 5,
    // interactive: false,
  ...
  locations.forEach(function (loc) {
    ...
    el.className = 'marker'; // note: 2) Add marker

    new mapboxgl.Marker({
      element: el,
      anchor: 'bottom'
    }).setLngLat(loc.coordinates).setPopup(new mapboxgl.Popup({
      offset: 30
    }).setHTML("<p>Day ".concat(loc.day, ": ").concat(loc.description, "</p>"))).addTo(map); // note: 3) Add Popup

    new mapboxgl.Popup({
      offset: 30
    }).setLngLat(loc.coordinates).setHTML("<p>Day ".concat(loc.day, ": ").concat(loc.description, "</p>")).addTo(map); // note: 4) Extend map bounds to include current locations
...
var stripe = Stripe('pk_test_51KdcWTJNoYmHoaT3dhqEUbd96kbNQOervmOPrVFcVf8IF4pT2LMFmA5fQaMuaBD6MDzCpDhzKYGg3ELpyQWCGBs8002dWE1rOw');
...
var bookBtn = document.getElementById('book-tour'); // note: VALUES
// note: DLEGATION
